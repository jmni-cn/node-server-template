...
async function bootstrap(): Promise<void> {
  ...
  const port = Number(config.get<string>('ADMIN_API_PORT') ?? 3001);
  await app.listen(port, '0.0.0.0');

  logger.log(`Admin API listening on http://localhost:${port}`);
}

bootstrap().catch((err) => {
  console.error('Admin API bootstrap failed:', err);
  process.exit(1);
});
