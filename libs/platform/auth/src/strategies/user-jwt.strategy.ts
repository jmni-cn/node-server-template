 ...
 * （锁定 HS256 算法）校验，随后：
 * 1. 校验 payload.typ==='user' 及必填字段（sub/jti 为 string、pv 为 number）；
 * 2. 校验 jti 不在 Token 黑名单（登出 / 强制下线）；
 * 3. 若绑定了 `ACCESS_SESSION_VALIDATOR` 端口，则校验会话/用户/密码版本实时有效；
 * 4. 将 payload 映射为 `UserAuthUser` 注入到 `request.user`。
 */
@Injectable()
export class UserJwtStrategy extends PassportStrategy(Strategy, 'user-jwt') {
  ...
  async validate(payload: UserJwtPayload): Promise<UserAuthUser> {
    // 1) 类型与必填字段校验。
    if (
      payload?.typ !== 'user' ||
